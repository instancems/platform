...
namespace InstanCeMS\Medialib\Exceptions;
...
class MediaUploadException extends Exception
{
    ...
    {
        return new static("Could not set adapter of class `{$class}`. Must implement `\InstanCeMS\Medialib\Adapters\SourceAdapterInterface`.");
    }

    public static function cannotSetModel($class)
    {
        return new static("Could not set `{$class}` as Media model class. Must extend `\InstanCeMS\Medialib\Models\Media`.");
    }
